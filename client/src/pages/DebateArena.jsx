...
const DebateArena = ({ examType }) => {
  ...
  const fetchDebate = async () => {
    setStage('loading');
    try {
      // 1. Get Topic (Priority 2 hook: weakest node)
      const topicRes = await fetch('https://reverse-tutor.onrender.com/api/graph/weakest');
      const topicData = await topicRes.json();
      setTopic(topicData.topic || 'Rotational Motion');

      // 2. Generate Debate Scenario
      const debateRes = await fetch('https://reverse-tutor.onrender.com/api/debate/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ topic: topicData.topic || 'Rotational Motion' })
      });
