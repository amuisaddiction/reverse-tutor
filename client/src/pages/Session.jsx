...
const Session = ({ topic, difficulty, onBack, examType }) => {
  ...
  // Priority 2: Feature Detection Fallback
  ...
  useEffect(() => {
    // Check for browser thread support
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      console.error(error);
      setMessages(prev => [...prev, { role: 'assistant', content: "Let's explore that further! Can you elaborate on the core principle?" }]); 
    }
    setLoading(false);
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-vercel-dark font-sans text-slate-300">
      ...
      {!sessionComplete && (
        <div className="p-6 bg-[#0A0F1E] border-t border-vercel-border shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.3)] z-20 flex justify-center flex-col items-center">
          
          {/* Priority 2: Safari / iOS Feature Detection Fallback */}
          {!hasSpeechSupport && (
            <div className="w-full max-w-3xl mb-4 bg-orange-500/10 border border-orange-500/30 text-orange-400 p-3 rounded-lg text-sm flex items-center gap-2">
              <AlertCircle size={16} /> ⚠️ Voice input requires desktop Chrome/Edge. Falling back to high-density text input.
            </div>
          )}
          ...
          <form onSubmit={handleSend} className="flex gap-4 w-full max-w-3xl relative">
            ...
            <input 
              ...
              placeholder={hasSpeechSupport ? (isListening ? "Listening... Speak now!" : "Type your explanation to correct the AI...") : "Type your explanation here..."}
              className={`flex-1 bg-vercel-dark border rounded-xl py-4 focus:outline-none focus:bg-vercel-card transition-all text-white placeholder-slate-500 ${
                hasSpeechSupport ? 'pl-14 pr-6' : 'px-6'
              } ${
